...
class DupCloudStorageAdapter extends AbstractStorageAdapter
{
    ...
    const MAX_PART_REQUEST_COUNT = 5;
    ...
    protected function realCopyToStorage(
        ...
        try {
            if ($this->isFile($storageFile)) {
                throw new Exception(sprintf('File already exists in cloud storage: %s', basename($storageFile)));
            }
            ...
            // 1. All the files that pass direct upload file type check will be uploaded direcly
            ...
            do {
                ...
                if (!isset($extraData['UploadUrls'][$partNumber - 1])) {
                    $partCount = $this->getRequestPartCount($offset, $length, filesize($sourceFile));
                    $result    = $this->client->getPartUrls(
                        $extraData['UploadUuid'],
                        $partNumber,
                        $partNumber + $partCount - 1
                    );

                    $extraData['UploadUrls'] = array_merge($extraData['UploadUrls'], $result);
                }

                if (!$this->client->uploadPart($extraData['UploadUrls'][$partNumber - 1], $content)) {
    ...
    public function copyFromStorage(
        ...
        try {
            ...
            if (is_file($destFile) && $offset === 0 && !unlink($destFile)) {
                throw new Exception('Can\'t delete destionation file');
            }
    ...
    /**
     * Returns the number of parts to request
     *
     ...
     * @return int
     */
    protected function getRequestPartCount(int $offset, int $length, int $filesize): int
    {
        $remainingParts = ceil(($filesize - $offset) / $length);

        return $remainingParts > self::MAX_PART_REQUEST_COUNT ? self::MAX_PART_REQUEST_COUNT : $remainingParts;
    }
