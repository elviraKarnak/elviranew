    ...
    public static function manualTransferStorageCallback(): array
    {
        ...
        try {
            ...
            foreach ($storage_ids as $storage_id) {
                ...
                // DupCloud doesn't support file overwrites, so we must prevent duplicate transfers
                // Other storage types handle overwrites automatically
                if (
                    $storage->getSType() === DupCloudStorage::getSType() &&
                    in_array($storage_id, $package->getValidStorages(true, 'id'))
                ) {
                    throw new Exception(__('The backup you are trying to transfer already exists in the cloud storage.', 'duplicator-pro'));
                }
                ...
                $upload_info = new UploadInfo($storage_id);
